...
1. For the Director
The Director should use these terms in its "Internal Monologue" to prune its reasoning.
	**Example:** "Initiating Munch-Init to establish savref of the repository structure."

2. For the Worker
Workers should use these terms to signal status back to the Director.
	**Example:** "I have achieved rek~mi of the task. Performing Munch-Grab on ID auth::validate#func."

3. For the Skill Creator
The Builder uses these terms to define the "Always Rules" in new skill files.
	**Example:** "Always use Munch-Search before performing a Munch-Grab."
