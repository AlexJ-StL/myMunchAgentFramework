...
- **Idempotency:** Always pass an `idempotency_key` when using the `create` IDs to prevent double-billing during agent retries.
...
- **Munch-Sync Notes:** *2026-04-12: Discovered that Stripe IDs starting with `cus_` must be treated as immutable Stable IDs in the `/wiki/entities` substrate.*
